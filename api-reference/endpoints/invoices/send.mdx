...
```bash cURL
curl -X POST https://app.contazen.ro/api/v1/invoices/inv_9z8y7x6w5v/send \
...
```javascript Node.js
const response = await fetch('https://app.contazen.ro/api/v1/invoices/inv_9z8y7x6w5v/send', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://app.contazen.ro/api/v1/invoices/inv_9z8y7x6w5v/send',
    json={
        'email': 'accounting@techsolutions.ro'  # optional
    },
...
```php PHP
$ch = curl_init('https://app.contazen.ro/api/v1/invoices/inv_9z8y7x6w5v/send');
