...
```bash cURL
curl -X PUT https://api.contazen.ro/v1/clients/9z8y7x6w5v \
...
const updateClient = async (clientId) => {
  const response = await fetch(`https://api.contazen.ro/v1/clients/${clientId}`, {
    method: 'PUT',
    headers: {
...
def update_client(client_id):
    url = f'https://api.contazen.ro/v1/clients/{client_id}'
    headers = {
        'Authorization': 'Bearer sk_live_YOUR_API_KEY',
        'Content-Type': 'application/json'
    }
...
<?php
$clientId = '9z8y7x6w5v';
$url = 'https://api.contazen.ro/v1/clients/' . $clientId;
$data = [
    'phone' => '+40722999888',
    'address' => 'Bd. Tehnologie nr. 100',
    'city' => 'București, Sectorul 2',
