...
<ParamField body="vat_breakdown" type="array">
  Update the per-rate VAT breakdown for multi-VAT receipts on the flat
  path. Send an array of `{ rate, net, vat_amount, gross }` entries (two or
  more). Ignored when `items` is supplied.
</ParamField>
