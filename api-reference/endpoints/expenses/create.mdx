...
Two save shapes are supported:

- **Flat** — send `amount` + `vat` for a single-line expense. Simplest path; covers most utility bills and single-rate receipts.
- **Itemized** — send `items[]` when the receipt has multiple line items or mixes VAT rates. The server derives `amount` / `vat` / `vat_breakdown` from the lines. See [Expense Lines & VAT Breakdown](/concepts/expense-lines) for the full model.

`amount` + `vat` are required **only** on the flat path — when `items` is present they become optional and are ignored in favor of the line-derived totals.
...
<ParamField body="vat" type="integer">
  VAT percentage (accepted values: 0, 5, 9, 11, 19, 21). **Required** on the flat path; when `items` is sent the server sets this to the dominant rate.
</ParamField>
...
<ParamField body="items" type="array">
  ...
  - `unit_price` (number, required) — **net** unit price (excluding VAT)
...
<ParamField body="vat_breakdown" type="array">
  Per-rate breakdown for multi-VAT receipts saved on the flat path (no
  `items`). Each entry: `{ rate, net, vat_amount, gross }`. The server stores
  the array as JSON on the row when two or more distinct rates are present.
  Ignored when `items` is supplied — in that case the breakdown is derived.
</ParamField>
